...
| D-19 | No email integration in prototype or original scope | Containerized Mailpit added to Docker Compose; emails sent for (1) one-pager delivery to a traveller on demand, and (2) automatic notification when a vacation request is Approved or Rejected | Highest-value bonus per assignment brief; demos fully offline with zero real SMTP credentials needed |
